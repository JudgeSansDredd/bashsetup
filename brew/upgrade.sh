...
choices=($(whiptail --title "Brew Upgrade" --separate-output --checklist "What brew installable apps would you like upgraded?" $BOX_HEIGHT $BOX_WIDTH 20 "${whiptail_brew_installable[@]}" 3>&2 2>&1 1>&3))
exitstatus=$?

if [ $exitstatus = 0 ]; then
    if [ ${#choices[@]} -gt 0 ]; then
        brew upgrade "${choices[@]}"
        brew cleanup
        brew doctor
    fi
fi
